...
contract LiquidityPoolManagerV2 is Ownable, ReentrancyGuard {
     ...
     * AVAX or PFX. Associates a weight with the pair. Rewards are distributed
     ...
     * and PFX pools regardless of liquidity. AVAX and PFX pools will
     * receive a fixed proportion of the pool rewards. The AVAX and PFX
     ...
     * the AVAX/PFX pool is a PFX pool. This method can also be used to
     * change the split ratio after fee splitting has been activated.
     *
     * Args:
     *   avaxSplit: Percent of rewards to distribute to AVAX pools
     *   pfxSplit: Percent of rewards to distribute to PFX pools
     */
    function activateFeeSplit(uint avaxSplit_, uint pfxSplit_) external onlyOwner {
        require(avaxSplit_.add(pfxSplit_) == 100, "LiquidityPoolManager::activateFeeSplit: Split doesn't add to 100");
        require(!(avaxSplit_ == 100 || pfxSplit_ == 100), "LiquidityPoolManager::activateFeeSplit: Split can't be 100/0");
        splitPools = true;
    ...
    function calculateReturns() public {
        ...
        if (pfxPairs.length() > 0) {
            ...
            for (uint i = 0; i < pfxPairs.length(); i++) {
                address pair = pfxPairs.at(i);
                uint pairLiquidity = getPfxLiquidity(pair, conversionRatio);
                uint weightedLiquidity = pairLiquidity.mul(weights[pair]);
                distribution[i + avaxPairs.length()] = weightedLiquidity;
                pfxLiquidity = SafeMath.add(pfxLiquidity, weightedLiquidity);
            }
        }
        ...
        uint transferred = 0;
        if (splitPools) {
            uint avaxAllocatedPfx = unallocatedPfx.mul(avaxSplit).div(100);
            uint pfxAllocatedPfx = unallocatedPfx.sub(avaxAllocatedPfx);

            for (uint i = 0; i < avaxPairs.length(); i++) {
                uint pairTokens = distribution[i].mul(avaxAllocatedPfx).div(avaxLiquidity);
            ...
            if (pfxPairs.length() > 0) {
                uint conversionRatio = getAvaxPfxRatio();
                for (uint i = 0; i < pfxPairs.length(); i++) {
                    uint pairTokens = distribution[i + avaxPairs.length()].mul(pfxAllocatedPfx).div(pfxLiquidity);
                    distribution[i + avaxPairs.length()] = pairTokens;
                    transferred = transferred.add(pairTokens);
                }
            }
        } else {
            uint totalLiquidity = avaxLiquidity.add(pfxLiquidity);
