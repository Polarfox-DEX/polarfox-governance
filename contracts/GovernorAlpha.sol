...
contract GovernorAlpha {
    ...
    string public constant name = "Polarfox Governor Alpha";

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint) { return 500_000e18; } // 500,000 PFX
    ...
    /// @notice The address of the Polarfox Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the Polarfox governance token
    PfxInterface public pfx;
    ...
    constructor(address timelock_, address pfx_, address guardian_) public {
        timelock = TimelockInterface(timelock_);
        pfx = PfxInterface(pfx_);
        guardian = guardian_;
    }

    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(pfx.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(pfx.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = pfx.getPriorVotes(voter, proposal.startBlock);
