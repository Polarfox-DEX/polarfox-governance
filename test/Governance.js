...
describe('Governance', function () {
    before(async function () {
        ...
        this.Wavax = await ethers.getContractFactory("Pfx");
        this.AltCoin = await ethers.getContractFactory("Pfx");
        ...
        this.Factory = await ethers.getContractFactory("PolarfoxFactory");
    ...
    beforeEach(async function () {
        ...
        this.pfxHandle2 = await this.pfx.connect(this.addr2);
        this.pfxHandle3 = await this.pfx.connect(this.addr3);
        ...
        // GovernorAlpha
        this.governor = await this.Governor.deploy(this.timelock.address, this.pfx.address, OWNER_ADDRESS);
        ...
        // Community Treasury
        this.community = await this.Community.deploy(this.pfx.address);
        ...
        this.lpManager = await this.LpManager.deploy(this.wavax.address, this.pfx.address,
    ...
    describe("Timelock Proposals", function () {
        ...
        it('Spend From Community Treasury', async function () {
            ...
            // Check preconditions
            await this.pfx.transfer(this.community.address, transferAmount);
            expect(await this.community.balance()).to.equal(transferAmount);
            expect(await this.pfx.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(0);
            ...
            await ethers.provider.send("evm_setNextBlockTimestamp", [updateTime]);
            await this.timelock.executeTransaction(target, value, sig, callData, startTime + DELAY);
            expect(await this.pfx.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(transferAmount);
    ...
    describe("Governor Proposals", function () {
        it('Change Admin', async function () {
            ...
            // Transfer PFX
            await this.pfx.transfer(this.addr2.address, this.proposalThreshold);
            await this.pfx.transfer(this.addr3.address, this.voteThreshold);

            // Delegate PFX so addresses can vote, must delegate to themselves
            await this.pfxHandle2.delegate(this.addr2.address);
            await this.pfxHandle3.delegate(this.addr3.address);
            await this.pfx.delegate(OWNER_ADDRESS);
