...
describe('TreasuryVester', function () {
    ...
    describe("Constructor", function () {
        it('pfx default', async function () {
            expect((await this.treasury.pfx())).to.equal(this.pfx.address);
        });
    ...
    describe("startVesting", function () {
        ...
        it('set claiming insufficient PFX', async function () {
            ...
            await expect(this.treasury.startVesting()).to.be.revertedWith(
                'TreasuryVester::startVesting: incorrect PFX supply');
        });
    ...
    describe("claim", function () {
        it('Claim once successfully', async function () {
            ...
            // Claim
            expect(await this.pfx.balanceOf(this.LP_MANAGER)).to.equal(ethers.constants.AddressZero);
            expect(await this.treasury.nextSlash()).to.equal(HALVING);
            await this.altContract.claim();
            expect(await this.pfx.balanceOf(this.LP_MANAGER)).to.equal(STARTING_AMOUNT);
        ...
        it('Claim twice successfully', async function () {
            ...
            await this.altContract.claim();
            expect(await this.pfx.balanceOf(this.LP_MANAGER)).to.equal(STARTING_AMOUNT.mul(2));
        ...
        it('Claim thrice successfully', async function () {
            ...
            await this.altContract.claim();
            expect(await this.pfx.balanceOf(this.LP_MANAGER)).to.equal(STARTING_AMOUNT.mul(3));
        ...
        //     const halving = 1;
        //     const interval = 1;
        //     const treasury = await this.TreasuryVester.deploy(this.pfx.address, STARTING_AMOUNT, halving, interval, TOTAL_AMOUNT);
        //     await treasury.deployed();
        //     altContract = await treasury.connect(this.altAddr);

        //     // Transfer initial PFX
        //     await this.pfx.transfer(treasury.address, TOTAL_AMOUNT);
        //     expect(await this.pfx.balanceOf(treasury.address)).to.equal(TOTAL_AMOUNT);
        ...
        //     expect(await this.pfx.balanceOf(this.LP_MANAGER)).to.equal(totalReceived);
        ...
        //     // const treasury = await this.TreasuryVester.deploy(this.pfx.address);
        //     // await treasury.deployed();
        //     // altContract = await treasury.connect(this.altAddr);

        //     // Transfer initial PFX
        //     await this.pfx.transfer(treasury.address, totalAmount);
        //     expect(await this.pfx.balanceOf(treasury.address)).to.equal(totalAmount);
        ...
        //     expect(await this.pfx.balanceOf(this.LP_MANAGER)).to.equal(startAmount);
        //     expect(await treasury.nextSlash()).to.equal(HALVING - 1);

        //     await ethers.provider.send("evm_increaseTime", [INTERVAL]);

        //     await expect(altContract.claim()).to.be.revertedWith("Pfx::_transferTokens: transfer amount exceeds balance");
        // });
    });
